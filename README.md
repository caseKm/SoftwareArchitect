...
# 重要なスキル
広い業務内容をカバーするために特有のスキルが必要だ。個人的経験や本、議論からソフトウェアアーキテクトが持つべきスキルは次の10個に絞りこめる:
  * Design
  * Decide
  * Simplify
  * Code
...
## (1) 設計
何が良い設計を作るか？これこそが最も重要で挑戦的な質問だろう。理論と実践を区別しよう。個人的経験からすると両方の要素を持つことが重要だ。まずは理論から始めよう:

* **基本設計パターンを知ろう**: パターンは、管理しやすいシステムを開発するためにアーキテクトが持つべき道具の１つである。パターンを使えば、デザインを再利用して、実績のある解決策で一般的な問題を解決することができます。ジョン・ブリシディース、ラルフ・ジョンソン、リチャード・ヘルム、エリック・ガンマによる書籍「オブジェクト指向における再利用のためのデザインパターン」はソフトウェア開発に関わる全員の必読書である。このパターンは20年以上前に発表されたものだが、今でも現代のソフトウェアアーキテクチャの基礎である。例えば、本書で説明されているModel-View-Controller (MVC) パターンは、多くの分野で応用されているか、あるいは新しいパターン(例えば Model-View-ViewModel (MVVM))の基礎となっている。
* **パターンとアンチパターンを突き詰める**: もしすでに基本的なGang-of-Fourのパターンをすべて知っているのであれば、より多くのソフトウェア・デザイン・パターンで知識を広げたり、興味のある分野をより深く突き詰めるべきだ。アプリケーション統合に関する私のお気に入りの本の一つは、Gregor Hohpe著の「Enterprise Integration Patterns」だ。この本は、2つのアプリケーションがデータを交換する必要があるときはいつでも、それがレガシーシステムからの古風なファイル交換であろうと、最新のマイクロサービスアーキテクチャであろうと、さまざまな分野で適用できる。
* **品質対策を知る**: アーキテクチャを定義して終わりではない。There are reasons why guidelines and coding standards are defined, applied and controlled. You do this because of quality and non-functional requirements. You want to have a system which is maintainable, reliable, adaptable, secure, testable, scalable, usable, etc. And one piece to achieving all of these quality attributes is applying good architecture work. You can start to learn more about quality measures on Wikipedia.
